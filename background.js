...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  ...
  // d) Cloud summarization (Anthropic Claude) - dedicated for HSK-level summaries
  if (request.action === 'summarizeWithCloudLLM') {
    chrome.storage.sync.get(['anthropicApiKey'], async (result) => {
      ...
      try {
        console.log('Calling Claude API for summarization with HSK level:', request.hskLevel);
        ...
        if (!data.content || !data.content[0] || !data.content[0].text) {
          sendResponse({ success: false, error: 'Unexpected response format from Claude API' });
          return;
        }

        const summary = data.content[0].text;
        console.log('Claude summary generated:', summary.substring(0, 100));
